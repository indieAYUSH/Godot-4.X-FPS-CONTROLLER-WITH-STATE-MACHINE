...
@export_group("lens distortion params")
@export var min_zoom : float = 20.0
@export var max_zoom : float = 170.0
@export var lens_distortion_rct : ColorRect
...
func _process(delta: float) -> void:
	...
	var desired_lens_zoom : float = clamp(remap(speed , min_speed , (max_speed*1.1) , min_zoom , max_zoom) , min_zoom , max_zoom)
	...
	current_lens_zoom = lerp(current_lens_zoom , desired_lens_zoom , smoothing_sped*delta)
